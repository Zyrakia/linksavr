...
export function createService<C, A extends Actions<C>>(client: C, actions: A): Service<C, A> {
	function wrap<P extends unknown[], R extends Result<unknown, unknown>>(action: Action<C, P, R>) {
		return async (...args: P) => {
			try {
 ...
 */
export function unwrapOrDomain<T>(result: Result<T, unknown>, onDomainError: (message: string) => never): T;
